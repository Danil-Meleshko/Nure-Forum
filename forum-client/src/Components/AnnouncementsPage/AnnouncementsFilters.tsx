...
export default function AnnouncementsFilters({ownAnnouns} : {ownAnnouns: boolean}) {
    ...
    const {getAnnouncements, allAnnouncementsOfTheUser} = useAnnouncementContext();
    const didRun = useRef(false);
    
    useEffect(() => {
        if(didRun.current){
            const timeOutId = setTimeout(() => {
                if(!ownAnnouns){ getAnnouncements(filters); }
                else allAnnouncementsOfTheUser(filters);
            }, 500);
